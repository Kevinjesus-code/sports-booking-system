...
// Detecta si la URL contiene ?token= (o &token=) al arrancar la app.
...
function App() {
  ...
  const [currentFlow, setCurrentFlow] = useState(() => {
    // Primero: ¿hay un token de reset en la URL?
    if (getResetToken()) return "reset-password";

    // Segundo: ¿hay sesión guardada?
    try {
  ...
  const handleLogout = () => setCurrentFlow("login");

  // Cuando el reset termina (éxito o cancelación) limpiamos el token
  // y quitamos el parámetro de la URL sin recargar la página.
