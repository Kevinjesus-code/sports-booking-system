...
const buildUser = (u) => ({
  ...
  role:     u?.rol ?? u?.role ?? "Recepcionista",
...
const Recepcionist = () => {
  ...
  const [userData, setUserData]                 = useState(() => buildUser(user));
  const [profileForm, setProfileForm]           = useState(() => buildUser(user));
  const [isEditingProfile, setIsEditingProfile] = useState(false);

  useEffect(() => {
    setUserData(buildUser(user));
    setProfileForm(buildUser(user));
  }, [user]);
            ...
            <ul className={styles["notifList"]}>
              {notifs.map((n) => (
                <li key={n.id} className={`${styles["notifItem"]} ${n.unread ? styles["unread"] : ""}`}>
                  ...
                  <div className={`${styles["notifIcon"]} ${n.iconClass}`}>{n.icon}</div>
          ...
          <div className={`${styles["panel"]} ${styles["profilePanel"]}`} onClick={(e) => e.stopPropagation()}>
            ...
            {isEditingProfile && (
              <div className={styles["profileEditForm"]}>
                ...
                <label>
                  Correo
                  <input type="email" value={profileForm.email} onChange={(e) => updateProfileForm("email", e.target.value)} />
                </label>
                <label>
                  Teléfono
            ...
            <div className={styles["profileActions"]}>
              ...
              <button className={`${styles["profileBtn"]} ${styles["logout"]}`} onClick={handleLogout}>
